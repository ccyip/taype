(* In this example, we show how we can use the oblivious libraries generated by
   the Taypsi compiler, and how we test our examples. Note that most of the
...
(* This is the oblivious library generated by the Taypsi compiler. It also
...
let () =
  ...
  (* An example of the input format is available in files
     "test_elem.alice.input" and "test_elem.bob.input". Our test framework takes
     a csv file (e.g., "test_elem.input.csv") that specifies multiple test cases
     and generates input and run the program in batch. The next few lines of
     code read input. All input is in s-expression. *)
  ...
  (* Obtain an oblivious list. The party who owns the private list reads it from
     ...
     used to convert the input s-expression to the list defined in the generated
     Taypsi library. In this case, [mylist_of_sexp_check] also checks the public
     view [n] is valid, i.e. it is not smaller than the actual size of the
     list. *)
  let xs =
    get_private (mylist_of_sexp_check n) (Conceal.obliv_list_s n)
      (Conceal.obliv_list_s_for n)
  in
  (* Obtain an oblivious integer, which is the element for the membership
     check. *)
