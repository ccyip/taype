...
module M (Driver : Taype_driver.S) = struct
  ...
  let features_to_sexp xs =
    features_to_list xs |> Conv.sexp_of_list Conv.sexp_of_int

  let features_of_sexp s =
    Conv.list_of_sexp Conv.int_of_sexp s |> features_of_list
  ...
  let rec spine_to_sexp = function
    | SNode (l, r) -> Sexp.List [ spine_to_sexp l; spine_to_sexp r ]
    | SLeaf -> Sexp.List []

  let rec spine_of_sexp = function
    | Sexp.List [ l; r ] -> SNode (spine_of_sexp l, spine_of_sexp r)
    | _ -> SLeaf
  ...
  let rec dtree_check_spineF t = function
    | SFNode (idx, l, r) -> (
        ...
        | Leaf _ -> false)
    | SFLeaf -> ( match t with Node _ -> false | Leaf _ -> true)
  ...
  let rec dtree_check_spine t = function
    | SNode (l, r) -> (
        ...
        | Leaf _ -> false)
    | SLeaf -> ( match t with Node _ -> false | Leaf _ -> true)

  let dtree_check_max t k = dtree_depth t <= k

  let dtree_of_sexp_check_spineF =
    of_sexp_check dtree_of_sexp dtree_check_spineF
